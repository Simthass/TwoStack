  ...
  const handleClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    if (href.startsWith('#')) {
      ...
      if (element) {
        ...
        // If section doesn't exist (might be on different page), navigate to home with hash
        router.push(`/${href}`);
      }
    } else {
...
export default function Nav() {
  ...
  const [scrollUpTimer, setScrollUpTimer] = useState<NodeJS.Timeout | null>(null);
  ...
  const whatsappLink = "https://wa.me/94767732288?text=Hi%20Two%20Stack%2C%20I%27d%20like%20to%20book%20a%20call%20to%20discuss%20my%20project.";
  ...
  return (
    <>
      ...
      >
        {/* Mobile hamburger — left column on mobile, hidden on desktop */}
        <button
          onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
          className="md:hidden flex flex-col justify-center gap-[5px] p-1 justify-self-start z-50"
          aria-label={isMobileMenuOpen ? "Close menu" : "Open menu"}
        >
          <span className={`block w-[22px] h-[2px] bg-white transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'rotate-45 translate-y-[5px]' : ''}`} />
          <span className={`block w-[22px] h-[2px] bg-white transition-opacity duration-300 ease-in-out ${isMobileMenuOpen ? 'opacity-0' : ''}`} />
          <span className={`block w-[22px] h-[2px] bg-white transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? '-rotate-45 -translate-y-[5px]' : ''}`} />
        </button>
        ...
        {/* Mobile WhatsApp button — right column on mobile, hidden on desktop */}
        <motion.a
          href={whatsappLink}
          target="_blank"
          rel="noopener noreferrer"
          className="md:hidden flex items-center gap-1.5 bg-[#25D366] text-white font-inter text-[10px] sm:text-[11px] font-medium px-2.5 sm:px-3.5 py-1.5 rounded-full flex-shrink-0 justify-self-end whitespace-nowrap"
          whileTap={{ scale: 0.95 }}
          onClick={(e) => {
            const target = e.currentTarget;
            target.style.backgroundColor = '#1DA851';
            setTimeout(() => {
        ...
        {/* Book a Call Button - Larger (desktop only) */}
        <motion.a
          href={whatsappLink}
          target="_blank"
          rel="noopener noreferrer"
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-40 bg-[#0a0a0a] md:hidden flex flex-col items-center justify-center px-6"
          >
            {/* Close button */}
            <button
            ...
            <motion.a
              ...
              className="mt-14 inline-flex items-center gap-2.5 bg-[#25D366] text-white font-inter font-medium text-sm px-8 py-3.5 rounded-full hover:bg-[#1DA851] transition-all duration-300"
            >
              <svg 
                width="18" 
                height="18" 
            ...
            >
              <a href="mailto:hello@twostack.com" className="hover:text-white/60 transition">twostacklk@gmail.com</a>
